...
if SavedScan.all.empty?
  SavedScan.create([{ name: 'ARP scan', parameter: '-sP -PR' },
                    ...
                    { name: 'Quick Java RMI / JMX scan', parameter: '-n -p 1090,1098,1099,1090,1199,4443,4444,4445,4446,8901,8902,8903,8999,9000,9001,9002,9003,9004,9005,9006,9007,9008,9009,9010,9999 -sV --script rmi-vuln-classloader,rmi-dumpregistry -T4 --max-retries 2 --script-timeout 15s' },
                    { name: 'Quick FTP-Anonymous detection', parameter: '-n -p 21 --script ftp-anon -T4 --max-retries 2 --script-timeout 15s' },
                    { name: 'Quick NFS-Mounts', parameter: '-n -p 111 --script=nfs-ls,nfs-showmount --max-retries 2 --script-timeout 40s' },
